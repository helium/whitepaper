...
\todo{Not sure where this belongs}
\newline
\textbf{Target Selection}. Due to the way scoring is structured, there is a possibility that honest miners may become stale as they might not get verified within recommended intervals. In order to overcome this limitation, we structure the target selection mechanism to give miners a reasonable chance to gain back trust and get verified by other miners. This is accomplished by biasing the probability of miners being selected as potential targets based on their individual scores.
...
The above equation ensures that the miner with the lowest score gets the highest probability of being selected as a potential target while the opposite holds for the miner with the highest scores.

Furthermore, it also asserts that the probabilities are inversely proportional to the score of individual miner.

This allows us to successfully target potentially low scoring miners and strengthen the fairness of the scoring system.
