...
%\authorinfo{Helium Systems, Inc.}{February 7, 2018}{}
\authorinfo{Amir Haleem\and Andrew Allen \and Andrew Thompson \and Marc Nijdam \and Rahul Garg}{Helium Systems, Inc.}{February 7, 2018}
...
\begin{enumerate}
  ...
  \item This process repeats with additional layers added to $O$ until all $T_n$ have a layer $O_l$ included in $O$
\end{enumerate}

The resulting $O$ can be visually represented as the following:

\todo{NEW DIAGRAM NEEDED, should show T1, T, and TL with T in the center. Also, each layer of O is now a three-tuple of $S, \psi, R$}

\begin{figure}[H]
    \begin{center}
          \includegraphics[width=\columnwidth]{diagram3.eps}
...
\begin{enumerate}
  ...
  \item If successful, $T_1$ then creates signed receipt $K_S$, ${K_S = \left(S || \beta || \upsilon\right)}$ signed by the private key of $T_1$
...
\todo{THIS DIAGRAM SHOULD ALSO BE UPDATED TO BETTER REPRESENT THE T1 -\textgreater T -\textgreater TL TYPE OF FLOW. ALSO THE SIGNED RECEIPTS ARE NOW IN THE FORM OF ${K_S = \left(S || \beta || \upsilon\right)}$}

\begin{figure}[H]
    \begin{center}
          \includegraphics[width=\columnwidth]{diagram4.eps}
...
\todo{PETE CAN WE MAKE THESE ANY EASIER TO READ? NOT A HUGE DEAL}
\begin{figure}[H]
    \begin{center}
          \includegraphics[width=0.8\columnwidth]{graph.eps}
...
\begin{figure}[H]
    \begin{center}
          \includegraphics[width=0.8\columnwidth]{graph10000.eps}
...
\todo{PETE - WORTH HAVING A DIAGRAM HERE?}
...
We believe that this tight integration of real-world hardware with a blockchain and a native token is a novel and valuable innovation that can be applied to other kinds of networks and wireless physical layers. We believe that the future of blockchains is not about who has the most hashing power or access to the cheapest electricity, but about blockchains where the mining proof is tied to providing a valuable, verifiable service. We plan to investigate or partner with interested parties to extend our ideas into other physical layers such as WiFi, Bluetooth and Cellular, as well as explore the potential for the delivery of 5G 60GHz+ mmWave connectivity through a similar mechanism. Additionally we plan to continue to research and implement more \emph{Proofs-of-Coverage} to keep the network secure as it grows.
\acks
...
\bibitem{bitcoin}
    Satoshi Nakamoto. 
    \emph{Bitcoinn: A peer-to-peer electronic cash system}, 2008
