...
\begin{enumerate}
  ...
  \item The shortest path between $T_1$ to $T$ to $T_L$ is computed using Dijkstra's algorithm
...
$C$ expects to hear responses from $T_n$ within a time threshold $\lambda$, otherwise it considers the \emph{Proof-of-Coverage} to have concluded. Because $C$ is the only party with complete knowledge of $O$, it can assign upper bounds to the values for $TA$ and $RS$ and thus verify that each layer of $O$ was transmitted approximately where and when it was expected.
