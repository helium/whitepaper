...
Transactions in the Helium blockchain provide functionality that enables address-to-address transfers of Helium tokens, similar to many existing blockchain networks, but also provide a set of primitives that enable core functionality that is critical to the operation of a \verb|DWN|. In this section we address the philosophy behind our transaction system, the primitives, and the way fees on the network work. We will also address Helium's need for microtransactions and propose a new solution.
...
Registers a new gateway on the network, adding it to an existing account which will be responsible for supplying its stake (required for mining) and will receive mining rewards and fees earned by the gateway
...
An assertion of a gateway's location in the form of geographic coordinates
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l X}
      ...
      lat & the latitude of the gateway \\
      lng & the longitude of the gateway \\
...
A generic payment transaction, used to move tokens between two accounts
...
Creates a staked deposit which can later be fractionally issued and redeemed in the form of chits without incurring transaction fees. Any unused portion of the deposit will be burned after some time.
...
\todo{is expiry height a fixed value? or configurable?}

\subsubsection{chit}
Issues a chit against a previously staked deposit in payment for transport or other services
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{l X}
      ...
      nonce & a monotoically increasing integer \\
      packet\_reference & a derivative reference to the packet being paid for \\
      payee\_address & the address of the recipient \\
      signature & the signature of the depositor
  \end{tabularx}
\end{table}

\todo{what's the packet reference? can it include metadata that could be referenced in the blockchain later?}

\subsection{Fees}

Transaction fees are an essential part of most blockchain implementations. They incentivize miners to include a transaction in their draft block and ensure that spam transactions don't pollute the blockchain and network. All transactions on the \verb|DWN| require transaction fees, with the exception of the \textbf{chit} transaction (covered in the Microtransactions section). Transaction fees are determined by...

\todo{How do we determine fees? are they just arbitrary and determined by demand? how do we specify that?}
...
Could we simply remove fees? A true zero-fee blockchain would be frought with spam transactions. It would be trivial to write a script to pollute the blockchain with garbage transactions. Some ostensibly zero-fee blockchain implemetations get around this issue in clever ways, such as by offloading the work of processing and verifying transactions to the transactors (iota). But those implementations aren't perfect, for example, IOTA hasn't yet proved it is capable of operating fully decentralized. \todo{fill this out a bit more}
...
Payment in arrear, after the services have been rendered, is extremely risky in a decentralized pseudo-anonymous system. You don't know if the person who is supposed to pay you will ever pay you, nor do you know if they'll even have the money when the debt comes due. This model only works when the parties involved trust each other, or have some recourse to recover funds (lawyers!).

\subsubsection{Light Clients \& Full Nodes}

One requirement for the Helium Network is that all transactions occur on-chain. This means that the size of the full blockchain will grow quite large. This issue is compounded by the fact that all miners on the network are hardware gateways, limited in computation power and storage space.

We will solve this constraint by allowing mining nodes to operate as \textbf{light clients} on the blockchain, pruning old blocks as needed and keeping only the latest ledger values. They will communicate over the p2p network with \textbf{full nodes} which maintain a complete history of the blockchain to verify transactions.

This begs the question of who is responsible for operating full nodes, and what their incentive to do so is. Routers are software-only applications with access to scalable, cloud-based storage and will be required to operate full nodes in order to fulfill their stated purpose. Helium will operate a router that will make it easy for developers to launch products without needing to deploy their own router. However, many enterprise developers, who are required to maintain a higher standard of privacy, will want to host their own router. Together, these routers will form a network of full nodes capable of supporting resource constrained gateways and wallets operating light clients.


\subsubsection{A Proposed Solution Based on Staking \& Chits}

I propose they push their pile of chips on the table and proceed to issue IOUs against their pile of tokens until it is exhausted. These IOUs are linked to each other, the blockchain needs to have seen the previous ones before subsequent ones will be accepted. Each IOU references the sequence number, the amount to pay, the hash of the packet, etc. These IOUs are returned in response to a gateway offering a packet for delivery.

Because the gateway concerned wants to get paid for transport, it will add the IOU transaction to its mempool, along with any previous IOUs that it depends on. This means that several gateways are all incentivized to include these IOUs in their blockchain. The entity issuing IOUs can even use the final IOU in the transaction chain to incentivize a miner to include all the previous (or missing) IOUs in a block. If previous transactions are not included in the chain, nobody gets paid.

On the face of things, this is similar to how state channels work, but with a couple notable differences. State channels are one to one, this is one to many. Additionally, this keeps the record of the transactions on the chain. The crucial advantage here is that we can likely eliminate transaction fees for most, if not all of these payments. The first, staking transaction, can be sent along with the first IOU transaction, so the gateway is incentivized to add it to their block. If the first miner doesn't include it, the second IOU can be accompanied with the staking transaction and the first IOU when communicating with the next miner, etc. Eventually the advantage of including the transactions outweighs the disadvantages.

In theory, it is possible for the IOU issuer to spend all the staked tokens and not have anything left to incentivize a miner to include the final transaction. All the miners involved in the IOU chain have an incentive to get it included in a block, and because we have a fairer chance any individual miner winning a block, this should eventually clear.

Note that nobody gets paid until everybody gets paid. IOUs should not be issued after the ExpiryHeight. If the stake is not distributed N blocks (like a week?) after the ExpiryHeight, the stake is disbursed as much as possible and any remaining balance is destroyed. This provides an incentive for everyone involved to settle the balance. Entities allowing their remaining stake to be destroyed would also potentially lose standing among gateways.

\todo{Edit this section}

\todo{how does the redemption period after the expiration work? we want to balance not wanting to hold onto deposits for a long time with preventing gateways from getting screwed out of their owed balance if they aren't chosen for a block in time}


\todo{can a router close a deposit prior to expiration and reclaim the remainder? how would that work if it has issued chits that haven't been published yet?}

\todo{what would a geolocation transaction that consults multiple gateways look like?}
...
\todo{Other problems we want to address?}
