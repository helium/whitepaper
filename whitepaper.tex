...
\subsubsection{add\_gateway}
  ...
  \end{table}
    ...
    Creates a staked deposit which can later be fractionally issued and redeemed in the form of chits without incurring transaction fees. Any unused portion of the deposit will be burned at the expiry block height.
    ...
    \subsubsection{redeem\_chit}
    Redeems a chit issued against a previously staked deposit in payment for transport or other services
...
Transaction fees are an essential part of most blockchain implementations. They asdf and ensure that spam transactions don't pollute the blockchain and network. All transactions on the \verb|DWN| require transaction fees, with the exception of the \textbf{redeem} transaction (covered in the Microtransactions section). Transaction fees are determined by...
...
\subsubsection{Stake Deposit \& Chits}
...
\item A router $R$ creates a deposit $S$ of 500 tokens, expiring at block height $n + 10,080$ (1 week from now), by creating a stake\_deposit transaction

\item A gateway $G$ hears a packet broadcast by device $D$ belonging to $R$
\item $G$ and $R$ negotiate a price of 1 token for transport
\item $G$ completes its requested transport service and as payment, $R$ issues a chit $C_{0}$ with value 1 token against $S$
\item $G$ verifies the correctness of $C_{0}$ , creates a redeem\_deposit transaction, adds it to its mempool and broadcasts the transaction to the p2p network for inclusion in the next block
\item $G$'s account balance is now += 1 token and the balance of $S$ is 499 tokens
\item $R$ continues issuing chits $C_{n}$  against $S$ until either:
    \begin{enumerate}
