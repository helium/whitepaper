...
\textbf{Utilizing the Proven Time}. The proven time allows us to prove, with relative certainty, that some piece of data $D$ was created between the time of the previous block $PbT^t$ and $T^t$. $D$ in this case is the proof-of-coverage. The randomness used to compute $O$ and thus obtain the proof-of-coverage is tied to the previous block, which contains $PbT^t$. Thus we know that $D$ must have been constructed between $PbT^t$ and $T^t$. This ensures that the proof-of-coverage cannot be pre-computed and that that blocks in the blockchain must have a minimum spacing in time; the \emph{blocktime}. Blocks which do not have at least \emph{blocktime} distance in time from their parent block are considered invalid.
...
An overview of the various steps in each part of the lifecycle of a miner is given in Cycle~\ref{cycle:miner}

\begin{algorithm}[H]
  \SetAlgorithmName{Cycle}{List of Lifecycles}
  \DontPrintSemicolon
  \SetKwProg{B}{\BlankLine Miner}{}{}
  \caption{Miner Cycle Overview}\label{cycle:miner}

  \B{At every epoch \emph{blocktime}} {
    Construct a block\;
  ...
  \B{At any time} {
    ...
    Receive block to add to chain or harvest\;
  ...
  \B{when joining} {
    ...
    Submit transaction to a random peer\;
  }
...
Now that we have discussed the requirements of the Helium Network, let's look at the existing solutions for micropayments on a blockchain and address their shortcommings as they apply to our network.

\paragraph{Heavyweight Transactions}
The first option is fine for larger transactions as the service fee is smaller than the payment. This doesn't work well for very small transactions as whoever pays the transaction fee ends up possibly paying more for the transaction fees than the value being exchanged. This is why you can't buy a packet of gum with a credit card. The gas station pays a minimum fee on each credit card transaction, and under a certain charge they lose money. This option is clearly not suitable for micro transactions.

\paragraph{Zero-fee Transactions}
If heavyweight fees aren't feasible, could we go to the other extreme and simply remove fees for payment transactions? Unfortunately, a true zero-fee blockchain would be frought with spam transactions. It would be trivial to write a script to pollute the blockchain with garbage transactions. Some ostensibly zero-fee blockchain implemetations get around this issue in clever ways, such as offloading the work of processing and verifying transactions to the transactors themselves \cite{iota}. But those implementations aren't perfect, for example, IOTA hasn't yet proved it is capable of operating in a fully decentralized manner.

\paragraph{State Channels}
State channels allow two parties to exchange value, small increments at a time, with very limited risk. If one party thinks the other is cheating them, they can publish the final transaction in the state channel to the blockchain and shut it down. At most one payment is usually at risk. However, there are several downsides: the payer has to lock up significant funds for the lifetime of the state channel, meaning they may be unable to open state channels with other parties or pay other dues; transactions in the state channel do not appear on the main chain at all; and they're relatively complex to get right (note that neither Lightning nor Raiden have become widely used yet).

\paragraph{Payment in Arrear}
Payment in arrear, after the services have been rendered, is extremely risky in a decentralized pseudo-anonymous system. You don't know if the person who is supposed to pay you will ever pay you, nor do you know if they'll even have the money when the debt comes due. This model only works when the parties involved trust each other, or have some recourse to recover funds (e.g. lawyers).

\subsubsection{A Proposed Solution Based on Staking \& Chits}

In order to our previously-stated requirement of on-blockchain microtransactions with zero fees, we propose a system that takes inspiration from staking, as well as state channels.

In order for a network user to issue zero fee payments to gateways, they must first stake a \emph{deposit} of tokens using the \emph{create\_deposit} transaction. This transaction has a fee. Once that deposit has been created, the network user can proceed to issue \emph{chits} against their deposit until it is exhaused. These chits are returned in response to a gateway offering a packet for delivery. Chits are linked to each other, the blockchain needs to have seen the previous ones before subsequent ones will be accepted. Each chit references the deposit, the sequence number (nonce), the amount to pay and any additional information about the packet.

Because the gateway concerned wants to get paid for transport, it will add the chit to its mempool, along with any previous chits that it depends on. This means that several gateways are all incentivized to include these chits in their blockchain. The entity issuing chits can even use the final IOU in the transaction chain to incentivize a miner to include all the previous (or missing) chits in a block. If previous chits are not included in the chain, nobody gets paid.

On the face of things, this is similar to how state channels work, but with a couple notable differences. State channels are one to one, this is one to many. Additionally, this keeps the record of the transactions on the chain. The crucial advantage here is that we can eliminate transaction fees for all of these payments. The first staking transaction can be sent along with the first chit, so the gateway is incentivized to add it to their block. If the first miner doesn't include it, the second IOU can be accompanied with the staking transaction and the first chit when communicating with the next miner. Eventually the advantage of including the transactions outweighs the disadvantages.

Note that nobody gets paid until everybody gets paid. Chits should not be issued after the ExpiryHeight. If the deposit is not distributed N blocks (like a week?) after the ExpiryHeight, the deposit is disbursed as much as possible and any remaining balance is destroyed. This provides an incentive for everyone involved to settle the balance. Entities allowing their remaining deposit to be destroyed would also potentially lose standing among gateways.

\subsubsection{Example Deposit Lifecycle}

In this section, we will walk through an example lifecycle of a router staking a deposit, gateways performing work on behalf of the router, and being compensated by the router through chits agaist the deposit.

\begin{enumerate}

\item A router $R$ creates a deposit $S$ of 500 tokens, expiring at block height $current\_height + 10,080$ (1 week from now), by publishing a create\_deposit transaction and paying a nominal transaction fee
\item A gateway $G_A$ hears a packet $P_1$ broadcast by device $D$
\item $G_A$ uses metadata attached to $P_1$, referencing the current state of the blockchain, to identify $R$ as the owner of $D$
\item $G_A$ sends the signature $Sig(P_1)$ of $P_1$ and an offer of 1 token for transport to $R$
\item $R$ receives $Sig(P_1)$ and the payment offer and can decide if it wants the packet for the offered price
\item Assuming $R$ wants the packet at the offered price, it constructs a \textbf{chit} transaction $C_1$ with nonce 1, value 1 token, payable to $G_A$, against $S$ and sends it to $G_A$
\item Having received $C_1$, $G_A$ verifies the correctness of $C_1$ by inspecting $S$ on the blockchain and sends $P$ to $R$
\item $G_A$ adds $C_1$ to its mempool and gossips it to the p2p network where any interested gateway can add it to its own mempool for inclusion in the next block
\item Meanwhile, $D$ has moved and when it broadcasts its next packet $P_2$, another gateway $G_B$ hears it
\item Just as before, $G_B$ sends $Sig(P_2)$ to $R$ along with an offer of 0.9 tokens for transport
...
\item $G_B$ adds $C_1$ and $C_2$ to its mempool and gossips both chit transactions to the p2p network
\item $G_A$, being invested in deposit $S$, hears about the new chit $C_2$ over the p2p network and adds it to its mempool
\item $R$ continues issuing chits $C_n$  against $S$ until either:
  \begin{enumerate}
  \item $S$'s value is depleted at which point $R$ can create a new deposit
...
Here, we address potential problems that could arise with the deposit \& chit approach and present solutions to these problems.

\paragraph{Degenerate Gateway} A gateway, after being issued a chit for the service it has agreed to perform, could neglect to follow through and perform the service. If a gateway fails to send the agreed upon packet to the router after the router issues a chit, the router can rerequest it (for some period of time). If the gateway is a bad actor and purposefully witholds the paid-for packet, the router may add the gateway to its own private blacklist of bad actor gateways. The threat of losing future business from a router and the relatively low overhead of completing the transfer of the packet acts to disincentivize gateways from acting in bad faith. This is consistent with state channel implementations where the task is subdivided such that the scope of potential loss is incredibly low.

\paragraph{Chit Bomb} A malicious actor could stake a deposit and then proceed to issue a near limitless amount of miniscule chits against that deposit without incurring transaction fees. This could flood mempools and pollute the blockchain with spam transactions. Assuming we trust our network of gateways (see trust/scoring section?), this could never happen. Gateways are the only mining entity on the network, and chits can only be issued to gateways in payment for their services. Because all gateways are selfish, they will only include transactions that benefit themselves. These are either transactions with fees attached, or chits that they have a direct investment in (either because they will get paid, or it's attached to a stake in which they have chits). Therefore, they would ignore any spam chits that were not issued in exchange for a service provided.

\paragraph{Final Transaction}
In theory, it is possible for the chit issuer to spend all the staked tokens and not have anything left to incentivize a miner to include the final transaction. All the miners involved in the chain of chits have an incentive to get it included in a block, and because there is a fair chance of any individual miner winning a block, this should eventually clear.

\subsection{Light Clients \& Full Nodes} \label{full nodes}

Up until now, we've discussed how to deal with microtransactions in a cost-effective way, however we haven't yet addressed how to deal with the size of the blockchain. One requirement we discussed for the Helium Network is that all transactions occur on-chain. This means that the size of the full blockchain will grow quite large. This issue is compounded by the fact that all miners on the network are gateways devices, limited in computation power and storage space.

We solve this constraint by allowing mining nodes to operate as \emph{light clients} on the blockchain, pruning old blocks and transactions as needed and keeping only the latest ledger values. They will communicate over the p2p network with \emph{full nodes} which maintain a complete history of the blockchain to verify transactions.

This raises a question: who is responsible for operating full nodes, and what is their incentive to do so? Routers are software-only applications with access to scalable, cloud-based storage and will be required to operate full nodes in order to fulfill their purpose. Helium will operate a router that will make it easy for developers to launch products without needing to deploy their own router. However, many enterprise developers, who are required to maintain a higher standard of privacy, will want to host their own router. Together, these routers will form a network of full nodes capable of supporting resource constrained gateways and wallets operating light clients.


\subsection{Fees}

Transaction fees are an essential part of most blockchain implementations. They incentivize miners to include a transaction in their draft block and ensure that spam transactions don't pollute the blockchain and network. We've discussed at length how the \emph{chit} transaction [\ref{chit}] is able to eliminate fees in the prior section on Microtransactions [\ref{microtransactions}]. However, all other transactions on the \verb|DWN| require transaction fees.

To determine the appropriate fee for a new transaction, the transactor will take the median of the past 1,000 packet transport fees, within some margin of error. Until 1,000 packet transports have been completed, the fee will be fixed at some constant. By anchoring the transaction fee to the current fees being charged for transport on the network, we root them in reality. Helium's primary purpose is to facilitate a network of IoT coverage. In order to accomplish this in the long term, all of the economics of the system must align to make it practical for the primary users (i.e. companies building and deploying devices) to transact on the network. If one set of fees were to outstrip the other, the network would quickly lose its utility for this key user segment.

To enable gateways and other light clients to determine an appropriate fee, full nodes [\ref{full nodes}] will expose a fee suggestion API. This way, resource constrained entities that don't maintain a complete copy of the blockchain will not need to compute the fee from the most recent transactions. When submitting a block, peer gateways will verify the correctness of the block and ensure that no fee deviates by more than the acceptable amount.

\subsection{Primitives in Helium}
Having discussed the philosophy of our transaction system and presented our approach to facilitate zero-fee mictrotransactions on the Helium network, we will finally delineate the transaction primitives and their properties.

\subsubsection{add\_gateway}

Registers a new gateway on the network, adding it to an existing account which will be responsible for supplying its stake (required for mining) and will receive mining rewards [\ref{mining}] and fees earned by the gateway
...
\subsubsection{assert\_location}

Asserts a gateway's location in the form of geographic coordinates
...
\subsubsection{payment} \label{payment}

Moves tokens from one account, the \emph{payer}, to another account, the \emph{payee}. N.B. this payment transaction requires a fee, while \emph{chit} [\ref{chit}] does not.
...
\subsubsection{create\_deposit}
Creates a staked deposit which can later be fractionally issued and redeemed in the form of \emph{chits} \ref{chit} without incurring transaction fees. Any unused portion of the deposit will be burned after some time.
...
\subsubsection{chit} \label{chit}
Issues a chit against a previously staked deposit in payment for transport or other services. N.B. this payment transaction does not require a fee, while \emph{payment} [\ref{payment}] does.
