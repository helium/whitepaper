...
\begin{tabular}{l l}
	...
	$\phi_0$,     & score of $\tau$ initially                      \\
	$\phi_h$,     & score of $\tau$ at height h                    \\
...
Similarly, the boost allotted to each gateway depends on the score and boost factor,
...
Substituting the values for $\alpha_h$ and $\epsilon_h$, we get the following recurrence relationship as the score of gateways:
...
The following graphs show a snapshot of a random subset of the network at any blockchain height \textit{h}. The nodes represent random locations with a score while the edges are calculated using the algorithm defined in \todo{refer dijsktra}
\begin{figure}[H]
    \begin{center}
          \includegraphics[width=0.5\textwidth]{graph.eps}
          \caption{\emph{Snapshot of a random subset of the network initially}}
          \label{fig:score-graph}
     \end{center}
\end{figure}

Following the scoring algorithm defined above and dijkstra's path traversal to compute the walk, we get the following graphs after multiple iterations. Each iteration represents \todo{refer poc-construction}
\begin{figure}[H]
    \begin{center}
          \includegraphics[width=0.5\textwidth]{graph10.eps}
          \caption{\emph{Snapshot of a random subset of the network after 10 iterations}}
          \label{fig:score-graph10}
     \end{center}
\end{figure}

\begin{figure}[H]
    \begin{center}
          \includegraphics[width=0.5\textwidth]{graph100.eps}
          \caption{\emph{Snapshot of a random subset of the network after 100 iterations}}
          \label{fig:score-graph100}
     \end{center}
\end{figure}

\begin{figure}[H]
    \begin{center}
          \includegraphics[width=0.5\textwidth]{graph1000.eps}
          \caption{\emph{Snapshot of a random subset of the network after 1000 iterations}}
          \label{fig:score-graph1000}
     \end{center}
\end{figure}
...
The goal of the above demonstrated algorithm is to ensure that the scoring of the gateways is done considering the fact that some gateways may act illegitimately, however, because the calculated edge-weights as defined in \todo{refer dijkstra 2.3.x} and the target selection mechanism ensure that we only boost the score of a miner when it is being verified by other high scoring gateways in a given subset; we believe that the system would work in order to favor legitimate gateways and act as a deterrent for illegitimate ones.
