...
Instead of an extremely computationally expensive and power hungry \emph{Proof-of-Work}, Helium miners generate \emph{Proofs-of-Coverage} \secref{poc} which allow them to partipciate in network consensus. In this section we present how we can use these useful proofs to create permisionless network consensus.
...
More recently there has been increased momentum around making blockchain consensus protocols less wasteful and more useful to the network. Filecoin \cite{filecoin} has a Proof-of-Spacetime and Ethereum \cite{ethereum} is moving towards a Proof-of-Stake approach.
...
\begin{itemize}
  ...
  \item \textbf{Transactions are censor-resistant}: ideally miners would not be able to censor or otherwise pick and choose transactions prior to mining them. This would not only nullify any attempts to nefariously censor transactions, but would allow for otherwise unattractive transactions (such as zero-fee transactions) to be included in the blockchain.
\end{itemize}
...
We describe this consensus protocol as the \emph{Helium Consensus Protocol (HCP)}.
