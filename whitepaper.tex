...
\begin{algorithm}
  ...
  \caption{Receive Protocol}\label{proto:recv}

  \Recv{transactions} {
    Add transactions to mempool\;
  }\label{proto:recv.trans}
...
\begin{algorithm}
  ...
  \caption{Receive Data Protocol}\label{proto:recv.data}

  \RecvData{at gateway G, from device D, data A, data signature $S_a$} {
    ...
    \Switch{\Deliver(R, $O$)} {
      \Case{${IOU}_{FG}$}{
        Validate ${IOU}_{FG}$\;
        \Deliver(R, A)\;
        $T \leftarrow \Transaction(redemption, G, {IOU}_{FG}) $\;
        Submit $T$ to peers\;
      }
      \lOther{drop A}
    }
  }\label{proto:recv.data.device}

  \RecvData{at gateway G, from router R, data A, for device D, with ${IOU}_{FG}$} {
