...
\begin{description}
  ...
  \item [Helium Network] We demonstrate an entirely new purpose-built block\-chain network built to service LongFi and provide a system for authenticating and identifying devices, providing cryptographic guarantees of data transmission and authenticity, offer transaction primitives designed around LongFi, and more.
  ...
  \item [\emph{Proof-of-Location}] We outline a system for interpreting the physical \emph{geolocation} of a Device using LongFi without the need for expensive and power-hungry satellite location hardware. Devices can make immutable, secure, and verifiable claims about their location at a given moment in time which is recorded in the blockchain.
...
\begin{itemize}
    ...
    \item Miners earn Helium tokens by providing wireless network coverage via comoodity hardware which provides a bridge between LongFi and Servers, which are Internet applications.
    ...
    \item Wireless data transmission costs are fixed and priced in Data Credits, which have a value targeting a fixed USD value. Each wireless data transmission (approximately 37 bytes of data) costs 1 Data Credit.
...
We introduce the core conmponents of the Peer-to-Peer Wireless Network.
...
\begin{description}
    \item [Devices] send and receive encrypted data from the Internet using hardware compatible with LongFi \secref{longfi}. There are hundreds of thousands of devices in the wild today capable of supporting LongFi. These devices can be updated via a firmware updated to communicate with the LongFi peer-to-peer wireless network.
...
\begin{description}
    \item [LongFi] The Helium network uses a new open wireless protocol, called \emph{LongFi}. LongFi is a long-range, low-power, wireless network protocol suitable for use with commodity hardware. LongFi compatible hardware can communicate over many square miles in dense urban environments or hundreds of square miles in rural settings. LongFi compatible hardware can also last for several years using standard batteries. LongFi uses strong public key cryptography and authentication occurs using the Helium blockchain, and data is encrypted end-to-end between the device and corresponding Internet-hosted server.

    \item [Hotspots] are physical network devices that provide wide-area wireless coverage and participate as Miners in the Helium network. Hotspots transmit data back and forth between Servers on the Internet and Devices while generating \emph{Proofs-of-Coverage} for the Helium network \secref{poc}. Hotspots are manufactured using commodity components. Hotspots can co-operate and geolocate Devices using the Helium network without the Device requiring any additional hardware. Each Hotspot can support thousands of connected Devices, and provide coverage over many square miles.
...
Satellite location information is also correlated with packet arrival events and can be used, in conjuction with other Hotspots who observe the same packet, to \emph{geolocate} devices that are not explicitly sending location information via GPS or some other means. This allows devices to locate themselves without requiring a GPS/GNSS transceiver, and therefore provide location data at a fraction of the battery life and cost of competing methods. More information on this method is described in \secref{geolocation}.

We will make both a complete open-source reference design and a finished Hotspot product available at launch of the Heilum network.
...
It is recommended that each Device use the Microchip ECC608A or equivalent hardware-based key storage device, which can securely generate, store, and authenticate public/private \emph{NIST P-256 ECC} \cite{nist} key pairs without leaking the private key. Also, a wide array of defense mechanisms prevent logical attacks on the encrypted data between the key storage device and its host Device, along with physical protections on the security device itself. Users program their key storage device as part of the onboarding process defined in the LongFi wireless specification using a defined API\@.

\subsection{Servers}

Servers are Internet-deployed applications that receive packets from Devices via Hotspots and route them to appropriate destinations such as an HTTP or MQTT endpoint.
...
When a Hotspot receives a data packet from a Device on the Helium network, it queries the blockchain to determine which Server to use given the Device's Helium network address. Anyone is free to host their own Server and define their Devices' traffic to be delivered there by any Hotspot on the Helium network. This ability allows users of the Helium network to create VPN-like functionality whereby encrypted data is delivered only to a Server (or set of Servers) that they specify and can optionally host themselves.
...
The protocol specification required for implementing a Server is defined in the LongFi Wireless Specification document that will be made available shortly.
...
Using \emph{Proof-of-Coverage} we achieve cryptographic proof of a Miners location. We can take advantage of these proofs to determine the physical \emph{geolocation} of LongFi Devices and generate a new type of proof based on the Devices geolocations. We call this \emph{Proof-of-Location}.
...
GPS has matured into an extraordinarily reliable service used in a wide range of applications for providing both location and time services. Several GPS transceivers are available at reasonable prices, some of which provide additional functionality oriented around low power applications. These include pre-loading the satellite location \emph{almanac} for faster location locks and improved low current operation. Additionally, it is possible with the correct Device hardware to \emph{sniff} WiFi base station MAC addresses and derive an approximate location via several publicly accessible databases. 

Lastly, we are able to employ a similiar GPS trilateration technique as described above, but in reverse using ground-based Hotspots with precise packet arrival timestamping to determine a Devices physical location. This last approach, commonly known as \emph{Time Differential of Arrival (TDoA)} has the benefit of the Device not requiring any additional location hardware. The Device simply sends a data packet using LongFi and its location can be derived by comparing the precise timestamps and known locations of multiple Hotspots who observe the same packet. This technique, however, is hindered by the presence of \emph{multipath propogation} where radio waves are reflected or refracted prior to arriving at their destination. Sources of multipath propogation in ground-based applications include buildings, cars, trees, humans and glass. Even with incredibly precise timestamping of 10 nanoseconds or below, such as the hardware available in the Helium Inc Hotspot v2 (arriving Q1 2020), multipath propogation creates a margin of error that can be anywhere from 30-300 meters depending on the density of Hotspots and clear line of sight to the Device.

\emph{Proof-of-Location} therefore intends to use the availability of multiple potential location techniques and the evidence obtained through \emph{Proof-of-Coverage} to provide cryptographic proof of a Devices location at the time a packet was received.
