...
\section{\textit{Proof-of-Coverage}}
...
In this section we outline the implementation for \textit{Proof-of-Coverage}.
...
\textit{Proof-of-Coverage} (\verb|PoC|) is an innovative proof which allows a miner, the prover $\mathcal{P}$, to convince a verifier $\mathcal{V}$ that wireless network coverage $\mathcal{W_p}$ is being made available to a given device on the network $\mathcal{D}$. \verb|PoC| is an interactive protocol where prover $\mathcal{P}$ asserts that they are creating wireless network coverage $\mathcal{W_p}$ in a specific GPS location $\mathcal{L_p}$ and then convinces a verifier $\mathcal{V}$ that $\mathcal{P}$ is in fact creating $\mathcal{W_p}$ via a challenge mechanism over the wireless network. \verb|PoC| is the first such protocol that attempts to prove the veracity of miners in a physical space, and then use it to achieve consensus on a blockchain network.\newline
