...
\begin{algorithm}
  ...
  \Recv{radio challenge envelope (challenger C, data O), at time TA, signal strength RS} {
    \Switch{\Decrypt(O, private key $E_{PK}$)} {
      \Case{Secret $S$, NextTime $TB$, Remainder $R$} {
        $K_s \leftarrow \ll S:64, TA:32, RS:32 \gg $\;
        $B \leftarrow \Sign(K_s, E_{pk}) $\;
        \Deliver(C, B)\;
        At time $TB$ \Transmit((C, R))\;
      }
...
\begin{algorithm}
  ...
  \RecvData{at gateway G, from device D, data A, data signature $S_a$} {
    ...
    Offer $O \leftarrow \Sign(\ll S_a, F \gg) $\;
    \Switch{\Deliver(R, $O$)} {
      \Case{${IOU}_{FG}$}{
        Validate ${IOU}_{FG}$\;
        \Deliver(R, A)\;
  ...
  \RecvData{at gateway G, from router R, data A, for device D, with ${IOU}_{FG}$} {
    \emph{This can only happen in resonse to G delivering data to R}
