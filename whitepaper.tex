...
\subsection{The Helium Network's Need for Microtransactions}
...
\item [Allow Devices to Persist Data to the Blockchain] Because the Helium network services a specific use, the \verb|DWN|, blocks must additionally be able to store fingerprints of data sent from Devices along with the transaction, which pays a Miner for its transport service.  We believe that this holistic \emph{tamper-proof} data trail will enable entirely new use cases where the authenticity and veracity of sensor data is critical.
