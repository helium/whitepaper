...
\textbf{Verifying the Proof}. \todo{FIX THIS SECTION TO NOT RANDOMLY TALK ABOUT PUBLISHING BLOCKS} Once the block is published to the network other miners can see the randomness, $\eta$ that was used by the challenger to generate the multi-layer-challenge. This allows them to do a parallel construction of the multi-layer-challenge and then verify the receipts for each target in the list.
