...
\begin{itemize}
    ...
    \item Miners specify the price they are willing to accept for data transport and \emph{Proof-of-Location} services, and machines specify the price they are willing to pay. Miners are paid once they prove they have delivered data to the machines' specified router.
...
\begin{itemize}
    \item \emph{Machines} pay miners to send encrypted data to and from the internet using hardware compatible with the Helium wireless protocol, called \emph{WHIP} \secref{whip}. In locations with a sufficient number of miners, machines can pay several miners to geolocate themselves without needing satellite location hardware, which we call \emph{Proof-of-Location}. Data sent from machines is \emph{fingerprinted}, and that fingerprint stored in the blockchain.
    ...
    \item \emph{Routers} are internet applications that receive encrypted machine data from miners. Routers are the termination point for machine data encryption. Machines record to the blockchain which routers miners should send their data to, such that any gateway on the network can send any machines data to the appropriate router. Routers are responsible for confirming to gateways that machine data was delivered to the correct destination and that the miner should be paid for their service.
\end{itemize}
