...
\begin{algorithm}[H]
  ...
  \Recv{data at gateway G, from device D, data A, data signature $S_a$} {
    $R \leftarrow R \in \FindRouters(D) $\;
    $F \leftarrow \CalculateDeliverFee(R, D, A) $\;
    \Switch{\DeliverOffer(R, $S_a$, F)} {
      \Case{${IOU}_{FG}$}{
        Validate ${IOU}_{FG}$\;
        \Deliver(R, A)\;
  ...
  \Recv{data at gateway G, from router R, data A, for device D, with ${IOU}_{FG}$} {
    \emph{This may happen in resonse to G delivering data to R}
