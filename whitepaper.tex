...
This whitepaper represents continuous work in progress. We will endeavor to keep this document current with the latest development progress. As a result of the on-going and iterative nature of our development process, the resulting code is likely to differ from what is represented in this paper.
...
In this section we discuss Helium's need for microtransactions, the limitations of existing solutions, and propose a solution based on the characteristics of our consensus protocol \secref{consensus}.
...
\subsubsection{A proposed solution enabled by the \emph{Helium Consensus Protocol}}

\todo{NEED TO WRITE SOMETHING HERE ABOUT HOW ZEROISH-FEE TRANSACTIONS FROM DEVICES COULD WORK}

\subsection{Transport Fees}

\todo{NEED TO WRITE SOMETHING HERE ABOUT HOW ZEROISH-FEE TRANSACTIONS FROM DEVICES COULD WORK?}

Devices using the Helium network to send and receive data to and from the internet must pay miners what is known as a \emph{transport fee}. This fee compensates the miner for delivering data packets between the device and the internet, and is unrelated to the \emph{transaction fee} that miners earn for mining transactions as part of blocks that are recorded to the blockchain.

Miners set the price they are willing to accept to transport data to and from the internet on a per-byte basis. 

Devices pay miners the transport fee on transmission or reception of the data. This means that the miner will receive payment the transport fee transaction prior to the transaction being mined in a block and recorded into the blockchain. This entails some risk for the miner, as they must believe that the transport payment is not malicious or fraudulent prior to it being confirmed in the blockchain. However, given how low it is expected the per-byte transport amount to be, this risk seems tolerable. A miner can \emph{blacklist} a device or organization if they continually abuse the system.

\subsection{Transaction Fees} \label{fees}
