...
\begin{enumerate}
	...
	\item \textbf{Blockchain Network}: we demonstrate an entirely new purpose-built blockchain network built to service and incentivize the \textit{Wireless Protocol} and provide a system for authenticating and identifying devices, providing cryptographic guarantees of data transmission and authenticity, offer transaction primitives designed around the wireless protocol, and more.	
	\item \textbf{Proof-of-Coverage}: we present a set of computationally-inexpensive \textit{Proof-of-Coverage} that allows network providers to prove they are capable of providing wireless network coverage. We anchor these proofs using a \textit{Proof-of-Serialization} that allows network providers to prove they are accurately representing time relative to others on the network in a crytographically secure way. 	
...
\begin{itemize}
	...
	\item The blockchain employs \textit{Proof-of-Coverage} to guarantee that miners are correctly representing the amount of wireless network coverage being created.
\end{itemize}

DIAGRAM OF THE SYSTEM HERE

\section{\textit{Proof-of-Coverage}} \label{poc}
...
In this section we outline the motivation and implementation of \textit{Proof-of-Coverage}.
...
We propose a consensus protocol that uses \textit{Proof-of-Coverage} to both secure the blockchain and provide an extremely useful service to the network; providing wireless network coverage that devices can use to send data to and from the internet. 

\subsection{\textit{Proof-of-Coverage}}

\textit{Proof-of-Coverage} (\verb|PoC|) is an innovative proof which allows a miner, the prover $\mathcal{P}$, to convince a verifier $\mathcal{V}$ that wireless network coverage $\mathcal{W_p}$ is being made available to a given device $\mathcal{D}$ on the network. \verb|PoC| is an interactive protocol where prover $\mathcal{P}$ asserts that they are creating wireless network coverage $\mathcal{W_p}$ in a specific GPS location $\mathcal{L_p}$ and then convinces a verifier $\mathcal{V}$ that $\mathcal{P}$ is in fact creating $\mathcal{W_p}$ via a challenge mechanism over the wireless network. \verb|PoC| is the first such protocol that attempts to prove the veracity of miners in a physical space, and then use it to achieve consensus on a blockchain network.\newline
...
\subsection{Constructing \textit{Proof-of-Coverage}}

This section describes the construction of the \textit{Proof-of-Coverage} protocol.\newline

\textbf{Selecting the Verifier}. Because Helium is a physical wireless network, we aim to create a proof that the prover $\mathcal{P}$ is creating wireless network coverage. To do this we must identify a verifier $\mathcal{V}$ that is within \textit{geographic proximity} to $\mathcal{P}$. We define geographic proximity as a miner present within $\mathcal{A}$ = $\mathcal{\pi}$$\mathcal{r^2}$; geographic proximity is required to confirm $\mathcal{P}$ and $\mathcal{V}$ are able to communicate via the RF wireless protocol. By communicating over the RF wireless protocol we confirm two critical things: 1) $\mathcal{P}$ and $\mathcal{V}$ are in fact geographically proximate to one another, and 2) that $\mathcal{P}$ is creating RF coverage compatible with the wireless protocol that devices are able to use. As each miners location is recorded in the blockchain via the \verb|assert_location| transaction (see Section \ref{transactions}) we select $\mathcal{V}$ by pseudo-random selection from miners located within $\mathcal{A}$ = $\mathcal{\pi}$$\mathcal{r^2}$ of $\mathcal{P}$, where $\mathcal{r}$ is approximately 1.5 miles from $\mathcal{P}$.\newline
