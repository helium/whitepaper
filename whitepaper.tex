...
current microtransaction implementations include state channels like lightning network and raiden. However, we see value to including metadata along with each packet transaction on the blockchain. And the purpose of state channels is to keep unnecessary transactions off the blockchain. big block approach, with light clients, pruning, routers serving as full nodes.

so this approach should help control the size of the blockchain, as gateways have fixed storage.

but what about fees? if we want to meter by packet, and for the cost of transport to be very cheap, we want to prevent the scenario where the transaction fees outstrip the transport fees themselves. In an ideal world we wouldn't have to charge two fees, one for providing transport and one for storing the transaction on the blockchain.

We propose a special type of transaction based on staking and burning.
...
\item A gateway $G$ hears a packet $P$ broadcast by device $D$
\item $G$ uses metadata attached to $P$, referencing the current state of the blockchain, to identify $R$ as the owner of $D$
\item $G$ sends the signature $Sig_P$ of $P$ and an offer of 1 token for transport to $R$
\item $R$ receives $Sig_P$ and the payment offer and can decide if it wants the packet for the offered price
\item Assuming $R$ wants the packet at the offered price, it constructs a \textbf{chit} transaction $C_0$ with nonce 0, value 1, payable to $G$, against $S$ and sends it to $G$
\item Having received $C_0$, $G$ verifies the correctness of $C_0$ by inspecting $S$ on the blockchain and sends $P$ to $R$
\item $G$ adds $C_0$ to its mempool and gossips it to the p2p network where any interested gateway can add it to its own mempool for inclusion in the next block
\item $R$ continues issuing chits $C_n$  against $S$ until either:
  ...
  \item $S$ expires at block height $n + 10,080$ and the remaining balance is burned
  \end{enumerate}
...
gateways are selfish and only add chits payable to them, or belonging to a deposit in which they have received chits, to their mempool.

this prevents spam attacks because only a trusted gateway that has performed some work in exchange for a chit will add it to their draft blocks

gateways gossip chits they receive to the p2p network of gateways

because no one gets paid until everyone gets paid, its in a gateway owed a chit's best interest to continue adding chits for that deposit to its draft blocks so that a deposit is exhausted sooner and it can get paid

chits must be added to the blockchain in order of nonce

if a router issues a chit with a higher nonce to a gateway, it can also include chits with nonces that make up the difference between what's already been added to the blockchain and its current nonce. This way, a gateway never has a partial nonce.

maybe a gateway could also ask a router for prior nonces in case it wasn't listening to gossip?

if a gateway fails to send the agreed upon packet to the router after the router issues a chit, the router can rerequest it (for some period of time). If the gateway is a bad actor and purposefully witholds the paid-for packet, the router may add the gateway to its own private blacklist of bad actor gateways. we hope that the threat of losing future business from a router and the relatively low overhead of completing the transfer of the packet will be enough to disincentivize gateways from acting in bad faith.
